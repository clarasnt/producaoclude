...
procurar.addEventListener("click", function(){
    ...
    var nomeCookie = document.cookie.split('; ')
    if(nomeCookie.find(row => row.startsWith(id+"_html")) != "undefined"){
        nomeCookie = nomeCookie.find(row => row.startsWith(id+"_html")).split('#')[1]
    } else {
